...
DapperGlib 1.4.0 provides targets for:
...
```bash
dotnet add package DapperGlib --version 1.4.0
```
...
Collections passed to `WhereIn` and `WhereNotIn` are sent through Dapper parameters instead of being manually concatenated into the SQL statement.
...
- Added parameterized query execution using Dapper bindings.
- Added typed collections support for `WhereIn` and `WhereNotIn`.
- Added `WhereLike` and `WhereContains`.
- Added generic `Value<T>()` and `Pluck<T>()`.
- Added `GetBindings()` for query parameter inspection.
- Added relationship `Create`, `CreateMany`, `Delete` and `DeleteAsync`.
- Added specific DapperGlib exceptions for clearer error handling.
- Added .NET 8.0 and .NET 10.0 targets while keeping .NET 6.0 compatibility.
