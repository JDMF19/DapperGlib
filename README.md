...
Version **1.4.5** expands the library with real asynchronous execution, `CancellationToken`, generic primary keys, global and per-query command timeouts, native Stored Procedures, `QueryMultiple`, `Single` / `SingleOrDefault`, and a new Laravel-inspired `DB` API for direct Dapper queries without leaving DapperGlib.

DapperGlib 1.4.5 provides targets for:
...
```bash
dotnet add package DapperGlib --version 1.4.5
```
...
`CreateManyAsync` executes the inserts asynchronously in sequence and supports `CancellationToken`.
...
Version **1.4.5** stabilizes the existing Query Builder core and expands Dapper integration while keeping the existing public API compatible.

- Real asynchronous execution using Dapper async APIs instead of blocking with `.Result` / `Task.FromResult`.
- Added `CancellationToken` overloads across async Query Builder, Model, Relationship, Stored Procedure and Raw operations.
- `WhereIn` / `WhereNotIn` use native Dapper collection parameters.
- Replaced identity retrieval based on `@@IDENTITY` with `OUTPUT INSERTED`.
- Added generic primary key support for `Find`, `FindAsync`, `FindOrDefault` and `FindOrDefaultAsync`.
- Generated keys are converted and assigned back to their native CLR property type.
- Centralized column validation and internal execution helpers.
- Improved exception consistency while preserving existing exception base types for compatibility.
- Added async `Insert`, `Create`, full/partial `Update`, `Value`, `Pluck`, `Count`, `Exists`, `DoesntExist`, `Max`, `Min`, `Avg` and `Sum`.
- Added `Relationship.CreateAsync` and `Relationship.CreateManyAsync`.
- Centralized command creation around Dapper `CommandDefinition`.
- Added optional global command timeout through `DapperGlib:CommandTimeout`.
- Added per-query / per-command `Timeout(seconds)`.
- Added `Single` and `SingleOrDefault`, including async versions and relationship support.
- Added Laravel-inspired `DB` API with default and named connections.
- Added native Stored Procedure execution with Input, Output, InputOutput and ReturnValue parameters.
- Added Stored Procedure `Execute`, `Scalar`, `First`, `FirstOrDefault`, `Single`, `SingleOrDefault`, `ToList` and async equivalents.
- Added `QueryMultiple` / `QueryMultipleAsync` and `MultipleResultReader`.
- Added Raw `Query`, `Execute`, `Scalar` and `QueryMultiple`, with sync / async variants.
- Raw and Stored Procedure APIs support DTOs without requiring `Model<T>`.
- Existing `Model.Raw(...)`, Query Builder APIs and legacy overloads remain available.

### #Compatibility Notes

Version 1.4.5 was implemented as an additive update wherever possible:

- Existing parameterless async method signatures remain available.
- New `CancellationToken` support is exposed through additional overloads.
- Existing `Find(int)` and `Find(string)` usage continues to work alongside generic primary key overloads.
- Existing exception inheritance was preserved so existing `catch` blocks continue to work.
- Existing Model-based APIs remain available; the new `DB` API is an additional lower-level path for direct Dapper access.

## Still is Dapper
DapperGlib continues to be built on top of Dapper. Version 1.4.5 provides the new `DB` API so many direct Dapper scenarios can now stay inside DapperGlib, but you can still access the underlying Dapper interface by instantiating the context when needed.
